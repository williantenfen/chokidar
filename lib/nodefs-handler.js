  ...
  if (!this.fsw.options.followSymlinks) {
    ...
    if (dir.has(item)) {
      if (this.fsw._symlinkPaths.get(full) !== linkPath) {
        ...
        this.fsw._emit('change', path, estats);
      }
    } else {
      dir.add(item);
      this.fsw._symlinkPaths.set(full, linkPath);
      this.fsw._emit('add', path, estats);
    }
    this.fsw._emitReady();
    return true;
  }
...
_handleRead(directory, initialAdd, wh, target, dir, depth, throttler) {
  ...
  const stream = this.fsw._readdirp(directory, {
    ...
    if (estats.isSymbolicLink() && await this._handleSymlink(entry, directory, path, item))
      return;
...
async _handleDir(dir, stats, initialAdd, depth, target, wh, realpath) {
  ...
  if ((oDepth == null || depth <= oDepth) && !this.fsw._symlinkPaths.has(realpath)) {
    ...
    closer = this._watchWithNodeFs(dir, (dirPath, stats) => {
      ...
      if (stats && stats.mtimeMs && stats.mtimeMs === 0) return;
